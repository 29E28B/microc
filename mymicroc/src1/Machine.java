...
public class Machine {
    ...
    static void execute(String[] args, boolean trace) throws FileNotFoundException, IOException, OperatorError, ImcompatibleTypeError {
        ...
        McBaseType[] stack = new McBaseType[STACKSIZE];

        McBaseType[] inputArgs = new McBaseType[args.length - 1];

        for (int i = 1; i < args.length; i++) {
            if(Pattern.compile("(?i)[a-z]").matcher(args[i]).find()){
                char[] input = args[i].toCharArray();
                McCharType[] array = new McCharType[input.length];
                for(int j = 0; j < input.length; ++j) {
                    array[j] = new McCharType(input[j]);
                }
                inputArgs[i-1] = new McArrayType(array);
            }
            else if(args[i].contains(".")){
                inputArgs[i-1] = new McFloatType(new Float(args[i]).floatValue());
            }
            else {
                inputArgs[i-1] = new McIntType(new Integer(args[i]).intValue());
            }
        }
    ...
    // 主执行函数
    private static int execCode(ArrayList<Integer> program, McBaseType[] stack, McBaseType[] inputArgs, boolean trace) throws ImcompatibleTypeError, OperatorError {
        int bp = -999;
        int sp = -1;
        int pc = 0;
        int hr = -1;
        for (;;) {
            ...
            switch (program.get(pc++)) {
                case Instruction.CSTI:
                    stack[sp + 1] = new McIntType(program.get(pc++)); sp++; break;
                case Instruction.CSTF:
                    stack[sp + 1] = new McFloatType(Float.intBitsToFloat(program.get(pc++))); sp++; break;
                case Instruction.CSTC:
                    stack[sp + 1] = new McCharType((char)(program.get(pc++).intValue())); sp++; break;
                ...
                case Instruction.DIV:
                    if(((McIntType)stack[sp]).getValue()==0)
                    {
                        System.out.println("hr:"+hr+" exception:"+1);
                        while (hr != -1 && ((McIntType)stack[hr]).getValue() != 1 )
                        {
                            hr = ((McIntType)stack[hr+2]).getValue();
                            System.out.println("hr:"+hr+" exception:"+new McIntType(program.get(pc)).getValue());
                        }
                            
                        if (hr != -1) { 
                            sp = hr-1;    
                            pc = ((McIntType)stack[hr+1]).getValue();
                            hr = ((McIntType)stack[hr+2]).getValue();    
                        } else {
                ...
                case Instruction.NOT: {
                    Object result = null;
                    if(stack[sp] instanceof McFloatType){
                        result = ((McFloatType)stack[sp]).getValue();
                    }else if (stack[sp] instanceof McIntType){
                        result = ((McIntType)stack[sp]).getValue();
                    }
                    stack[sp] = (Float.compare(new Float(result.toString()), 0.0f) == 0 ? new McIntType(1) : new McIntType(0));
                    break;
                }
                ...
                case Instruction.SWAP: {
                    McBaseType tmp = stack[sp];  
                    stack[sp] = stack[sp-1];  
                    stack[sp-1] = tmp;
                    break;
                }
                case Instruction.LDI:
                    stack[sp] = stack[((McIntType)stack[sp]).getValue()]; break;
                case Instruction.STI:
                    stack[((McIntType)stack[sp-1]).getValue()] = stack[sp]; stack[sp-1] = stack[sp]; sp--; break;
                case Instruction.GETBP:
                    stack[sp+1] = new McIntType(bp); sp++; break;
                case Instruction.GETSP:
                    stack[sp+1] = new McIntType(sp); sp++; break;
                ...
                case Instruction.IFZERO: {
                    Object result = null;
                    int index = sp--;
                    if(stack[index] instanceof McIntType){
                        result = ((McIntType)stack[index]).getValue();
                    }else if(stack[index] instanceof McFloatType){
                        result = ((McFloatType)stack[index]).getValue();
                    }
                ...
                case Instruction.CALL: {
                    ...
                    stack[sp-argc+1] = new McIntType(pc+1); sp++;
                    stack[sp-argc+1] = new McIntType(bp);   sp++;
                    bp = sp+1-argc;
                    pc = program.get(pc);
                    break;
                }
                ...
                case Instruction.RET: {
                    McBaseType res = stack[sp];
                    sp = sp - program.get(pc); bp = ((McIntType)stack[--sp]).getValue(); pc = ((McIntType)stack[--sp]).getValue();
                    stack[sp] = res;
                } break;
                case Instruction.PRINTI: {
                    Object result;
                    if(stack[sp] instanceof McIntType){
                        result = ((McIntType)stack[sp]).getValue();
                    }else if(stack[sp] instanceof McFloatType){
                        result = ((McFloatType)stack[sp]).getValue();
                    }else {
                        result = ((McCharType)stack[sp]).getValue();
                    }
                ...
                case Instruction.PRINTC:
                    System.out.print((((McCharType)stack[sp])).getValue()); break;
                ...
                case Instruction.PUSHHR:{
                    stack[++sp] = new McIntType(program.get(pc++));    //exn
                    int tmp = sp;       //exn address
                    sp++;
                    stack[sp++] = new McIntType(program.get(pc++));   //jump address
                    stack[sp] = new McIntType(hr);
                    hr = tmp;
                    break;
                }
                case Instruction.POPHR:
                    hr = ((McIntType)stack[sp--]).getValue();sp-=2;break;
                case Instruction.THROW:
                    System.out.println("hr:"+hr+" exception:"+new McIntType(program.get(pc)).getValue());

                    while (hr != -1 && ((McIntType)stack[hr]).getValue() != program.get(pc) )
                    {
                        hr = ((McIntType)stack[hr+2]).getValue(); //find exn address
                        System.out.println("hr:"+hr+" exception:"+new McIntType(program.get(pc)).getValue());
                    }
                    ...
                    if (hr != -1) { // Found a handler for exn
                        ...
                        pc = ((McIntType)stack[hr+1]).getValue();
                        hr = ((McIntType)stack[hr+2]).getValue(); // with current handler being hr     
                    } else {
                        System.out.print(hr+"not find exception");
                        return sp;
                    }break;
    ...
    // 执行二元运算符的操作
    public static McBaseType binaryOperator(McBaseType lhs, McBaseType rhs, String operator) throws ImcompatibleTypeError, OperatorError {
        Object left;
        Object right;
        int flag = 0;
        if (lhs instanceof McFloatType) {
            left = ((McFloatType) lhs).getValue();
            flag = 1;
        } else if (lhs instanceof McIntType) {
            left = ((McIntType) lhs).getValue();
        } else {
            throw new ImcompatibleTypeError("ImcompatibleTypeError: Left type is not int or float");
        }

        if (rhs instanceof McFloatType) {
            right = ((McFloatType) rhs).getValue();
            flag = 1;
        } else if (rhs instanceof McIntType) {
            right = ((McIntType) rhs).getValue();
        } else {
            throw new ImcompatibleTypeError("ImcompatibleTypeError: Right type is not int or float");
        }
        McBaseType result = null;

        switch(operator){
            case "+":{
                if (flag == 1) {
                    result =  new McFloatType(Float.parseFloat(String.valueOf(left)) + Float.parseFloat(String.valueOf(right)));
                } else {
                    result = new McIntType(Integer.parseInt(String.valueOf(left)) + Integer.parseInt(String.valueOf(right)));
                }
                break;
            }
            case "-":{
            ...
            case "==":{
                if (flag == 1) {
                    if((float) left == (float) right){
                        result = new McIntType(1);
                    }
                    else{
                        result = new McIntType(0);
                    }
                } else {
    ...
    // 输出栈式虚拟机现状
    private static void printSpPc(McBaseType[] stack, int bp, int sp, ArrayList<Integer> program, int pc) {
        System.out.print("[ ");
        for (int i = 0; i <= sp; i++) {
            Object result = null;
            if(stack[i] instanceof McIntType){
                result = ((McIntType)stack[i]).getValue();
            }else if(stack[i] instanceof McFloatType){
                result = ((McFloatType)stack[i]).getValue();
            }else if(stack[i] instanceof McCharType){
                result = ((McCharType)stack[i]).getValue();
            }
            System.out.print(String.valueOf(result) + " ");
        }
        System.out.print("]");
