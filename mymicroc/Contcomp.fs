...
let rec cStmt stmt (varEnv : VarEnv) (funEnv : FunEnv) (labellist : LabelEnv) (structEnv : StructTypeEnv) (C : instr list) : instr list = 
    match stmt with
    | If(e, stmt1, stmt2) -> 
      let (jumpend, C1) = makeJump C
      let (labelse, C2) = addLabel (cStmt stmt2 varEnv funEnv labellist structEnv C1)
      cExpr e varEnv funEnv labellist structEnv (IFZERO labelse 
       :: cStmt stmt1 varEnv funEnv labellist structEnv (addJump jumpend C2))
    | While(e, body) ->
      let labbegin = newLabel()
      let (jumptest, C1) = 
           makeJump (cExpr e varEnv funEnv labellist structEnv (IFNZRO labbegin :: C))
      addJump jumptest (Label labbegin :: cStmt body varEnv funEnv labellist structEnv C1)
    | DoWhile(body, e) ->    //do..while
        let labbegin = newLabel()
        let C1 = 
            cExpr e varEnv funEnv labellist structEnv (IFNZRO labbegin :: C)
        Label labbegin :: cStmt body varEnv funEnv labellist structEnv C1 //先执行body
    | DoUntil(body, e) ->    //do..until
        let labbegin = newLabel()
        let C1 = 
            cExpr e varEnv funEnv labellist structEnv (IFZERO labbegin :: C)
        Label labbegin :: cStmt body varEnv funEnv labellist structEnv C1
    | For(dec, e, opera,body) ->
        let labend   = newLabel()                       //结束label
        let labbegin = newLabel()                       //设置label 
        let labope   = newLabel()                       //设置 for(,,opera) 的label
        let lablist = labend :: labope :: labellist
    ...
    | Range(dec,i1,i2,body) ->
        ...
        let judge = Prim2 ("<",Access (tmp dec),i2)
        let opera = Assign (tmp dec, Prim2 ("+",Access (tmp dec),CstI 1))
        cStmt (For (ass,judge,opera,body))  varEnv funEnv labellist structEnv C
    | Switch(e,cases)   ->
        ...
        let rec everycase c  = 
            match c with
            | [Case(cond,body)] -> 
                let (label,C2) = addLabel(cStmt body varEnv funEnv labellist structEnv C1 )
                let (label2, C3) = addLabel( cExpr (Prim2 ("==",e,cond)) varEnv funEnv labellist structEnv (IFZERO labend :: C2))
                (label,label2,C3)
            | Case(cond,body) :: tr->
                let (labnextbody,labnext,C2) = everycase tr
                let (label, C3) = addLabel(cStmt body varEnv funEnv labellist structEnv (addGOTO labnextbody C2))
                let (label2, C4) = addLabel( cExpr (Prim2 ("==",e,cond)) varEnv funEnv labellist structEnv (IFZERO labnext :: C3))
                (label,label2,C4)
            | [] -> (labend, labend,C1)
    ...
    | Try(stmt,catchs)  ->
        ...
        let rec everycatch c  = 
            ...
            | [Catch(exn,body)] -> 
                ...
                let (label, Ccatch) = addLabel( cStmt body varEnv funEnv labellist structEnv [])
                let Ctry = PUSHHDLR (exnum ,label) :: tryins @ [POPHDLR]
                (Ccatch,Ctry)
            | Catch(exn,body) :: tr->
                let exnum = lookupExn exn exns 1
                let (C2,C3) = everycatch tr
                let (label, Ccatch) = addLabel( cStmt body varEnv funEnv labellist structEnv C2)
    ...
    | Catch(exn,body)       ->
        C
    | Expr e -> 
      cExpr e varEnv funEnv labellist structEnv (addINCSP -1 C) 
    | Block stmts -> 
      ...
      let rec pass2 pairs C = 
          ...
          | (BStmt stmt, varEnv) :: sr -> cStmt stmt varEnv funEnv labellist structEnv (pass2 sr C)
      pass2 stmtsback (addINCSP(snd varEnv - fdepthend) C)
    | Return None -> 
      RET (snd varEnv - 1) :: deadcode C
    | Return (Some e) -> 
      cExpr e varEnv funEnv labellist structEnv (RET (snd varEnv) :: deadcode C)
    | Break ->
      let labend = headlab labellist
      addGOTO labend C
    | Continue ->
      let labellist = dellab labellist
      let labbegin = headlab labellist
      addGOTO labbegin C

and tryStmt tryBlock (varEnv : VarEnv) (funEnv : FunEnv) (labellist : LabelEnv) (structEnv : StructTypeEnv) (C : instr list) : instr list * VarEnv = 
    match tryBlock with
    | Block stmts ->
        let rec pass1 stmts ((_, fdepth) as varEnv) = 
            match stmts with
...
and cExpr (e : expr) (varEnv : VarEnv) (funEnv : FunEnv) (labellist : LabelEnv) (structEnv : StructTypeEnv) (C : instr list) : instr list =
    match e with
    | Access acc     -> cAccess acc varEnv funEnv labellist structEnv (LDI :: C)
    | Assign(acc, e) -> cAccess acc varEnv funEnv labellist structEnv (cExpr e varEnv funEnv labellist structEnv (STI :: C))
    | CstI i         -> addCST i C
    | CstC i         -> addCSTC i C
    | CstF i         -> addCSTF i C
    | Addr acc       -> cAccess acc varEnv funEnv labellist structEnv C
    | ToInt e        -> match e with
                        |  CstF f -> addCST (int f) C
                        |  CstC c -> addCST (int c - int '0') C
    | ToChar e       -> match e with
                        |  CstI i -> addCSTC (char (i + int "0")) C                        
    ...
    | Prim1(ope, e1) ->
      let rec tmp stat =
                    match stat with
                    | Access (c) -> c               //get IAccess
      cExpr e1 varEnv funEnv labellist structEnv
          (match ope with
           | "!"      -> addNOT C
           | "printi" -> PRINTI :: C
           | "printc" -> PRINTC :: C
           | "I++" -> 
                ...
                cExpr ass varEnv funEnv labellist structEnv (addINCSP -1 C)
           | "++I" -> 
                let ass = Assign (tmp e1,Prim2 ("+",Access (tmp e1),CstI 1))
                let C1 = cExpr ass varEnv funEnv labellist structEnv C
    ...
    | Prim2(ope, e1, e2) ->
      cExpr e1 varEnv funEnv labellist structEnv
        (cExpr e2 varEnv funEnv labellist structEnv
           (match ope with
            | "*"   -> MUL  :: C
            | "+"   -> ADD  :: C
            | "-"   -> SUB  :: C
    ...
    | Prim3(cond, e1, e2)    ->
        let (jumpend, C1) = makeJump C
        let (labelse, C2) = addLabel (cExpr e2 varEnv funEnv labellist structEnv C1)
        cExpr cond varEnv funEnv labellist structEnv (IFZERO labelse :: cExpr e1 varEnv funEnv labellist structEnv (addJump jumpend C2))
    | Andalso(e1, e2) ->
      match C with
      | IFZERO lab :: _ ->
         cExpr e1 varEnv funEnv labellist structEnv (IFZERO lab :: cExpr e2 varEnv funEnv labellist structEnv C)
      | IFNZRO labthen :: C1 -> 
        let (labelse, C2) = addLabel C1
        cExpr e1 varEnv funEnv labellist structEnv
           (IFZERO labelse 
              :: cExpr e2 varEnv funEnv labellist structEnv (IFNZRO labthen :: C2))
      | _ ->
        let (jumpend,  C1) = makeJump C
        let (labfalse, C2) = addLabel (addCST 0 C1)
        cExpr e1 varEnv funEnv labellist structEnv
          (IFZERO labfalse 
             :: cExpr e2 varEnv funEnv labellist structEnv (addJump jumpend C2))
    | Orelse(e1, e2) -> 
      match C with
      | IFNZRO lab :: _ -> 
        cExpr e1 varEnv funEnv labellist structEnv (IFNZRO lab :: cExpr e2 varEnv funEnv labellist structEnv C)
      | IFZERO labthen :: C1 ->
        let(labelse, C2) = addLabel C1
        cExpr e1 varEnv funEnv labellist structEnv
           (IFNZRO labelse :: cExpr e2 varEnv funEnv labellist structEnv
             (IFZERO labthen :: C2))
      | _ ->
        let (jumpend, C1) = makeJump C
        ...
        cExpr e1 varEnv funEnv labellist structEnv
           (IFNZRO labtrue 
             :: cExpr e2 varEnv funEnv labellist structEnv (addJump jumpend C2))
    | Call(f, es) -> callfun f es varEnv funEnv labellist (structEnv : StructTypeEnv) C
...
and cAccess access varEnv funEnv labellist (structEnv : StructTypeEnv) C = 
    match access with 
    | AccVar x   ->
      match lookup (fst varEnv) x with
      | Glovar addr, _ -> addCST addr C
      | Locvar addr, _ -> GETBP :: addCST addr (ADD :: C)
    | AccessMember (AccVar stru, AccVar memb) ->
    ...
    | AccDeref e ->
      cExpr e varEnv funEnv labellist structEnv C
    | AccIndex(acc, idx) ->
      match acc with
        | AccessMember (AccVar stru, AccVar memb) ->
            cAccess acc varEnv funEnv labellist structEnv (cExpr idx varEnv funEnv labellist structEnv (ADD :: C))
        | _     ->   
            cAccess acc varEnv funEnv labellist structEnv (LDI :: cExpr idx varEnv funEnv labellist structEnv (ADD :: C))


and cExprs es varEnv funEnv labellist (structEnv : StructTypeEnv) C = 
    match es with 
    | []     -> C
    | e1::er -> cExpr e1 varEnv funEnv labellist structEnv (cExprs er varEnv funEnv labellist structEnv C)

    
and callfun f es varEnv funEnv labellist (structEnv : StructTypeEnv) C : instr list =
    let (labf, tyOpt, paramdecs) = lookup funEnv f
    let argc = List.length es
    if argc = List.length paramdecs then
      cExprs es varEnv funEnv labellist structEnv (makeCall argc labf C)
    else
