...
and stmtordec stmtordec locEnv gloEnv store =
    ...
    | DeclareAndAssign (typ, x, e) ->let (v1,store1) = allocate (typ, x) locEnv store
                                     let (v2, store2) = access (AccVar x) v1 gloEnv store1
                                     let (res, store4) = eval e v1 gloEnv store2
                                     (v1, setSto store4 v2 res) 
...
and eval e locEnv gloEnv store : int * store =
    ...
    | CstI i -> (i, store)
    | CstC c -> ((int c), store)
    | CstF i -> (System.BitConverter.SingleToInt32Bits(float32(i)), store)
    ...
    | Prim2 (ope, e1, e2) ->
        ...
        (res, store2)
    | Prim3( cond, e2 , e3) ->
        let (i1, store1) = eval cond locEnv gloEnv store
        let (i2, store2) = eval e2 locEnv gloEnv store1
        let (i3, store3) = eval e3 locEnv gloEnv store1
        if i1 = 1 then (i2,store2) 
                  else (i3,store3) 
    | Andalso (e1, e2) ->
